...
- **planning/** – Setup guides (`planning/setup/`), and for the initial scope: project overview and definition under `planning/00-initial-milestones/` (`project-overview/`, `definition/`). For new milestones, `planning/<NN-milestone>/project-overview` and `definition` can be added when the milestone has its own scope (see new-milestone-setup).
